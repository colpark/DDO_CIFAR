...
class SpectralConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, dim1, dim2, modes1=None, modes2=None, separable=False):
        ...
        if self.separable:
            self._weights1a = nn.Parameter(self.scale * (torch.randn(in_channels, out_channels, self.modes1, 1, dtype=torch.cfloat)))
            self._weights1b = nn.Parameter(self.scale * (torch.randn(in_channels, out_channels, 1, self.modes2, dtype=torch.cfloat)))
            self._weights2a = nn.Parameter(self.scale * (torch.randn(in_channels, out_channels, self.modes1, 1, dtype=torch.cfloat)))
            self._weights2b = nn.Parameter(self.scale * (torch.randn(in_channels, out_channels, 1, self.modes2, dtype=torch.cfloat)))
        else:
            self._weights1 = nn.Parameter(self.scale * (torch.randn(in_channels, out_channels, self.modes1, self.modes2, dtype=torch.cfloat)))
            self._weights2 = nn.Parameter(self.scale * (torch.randn(in_channels, out_channels, self.modes1, self.modes2, dtype=torch.cfloat)))
...
class UNO2d(nn.Module):
    def __init__(self, in_ch, d_co_domain, half_modes, pad=0, factor=3/4, out_ch=None, separable=False):
        super().__init__()
        ...
        self.fc0 = nn.Linear(2 + in_ch, self.d_co_domain) # input channel is 3: (a(x, y), x, y)

        self.conv0 = SpectralConv2d(self.d_co_domain, 2*factor*self.d_co_domain, 48, 48, half_modes, half_modes, separable=separable)

        self.conv1 = SpectralConv2d(2*factor*self.d_co_domain, 4*factor*self.d_co_domain, 32, 32, half_modes//2, half_modes//2, separable=separable)

        self.conv2 = SpectralConv2d(4*factor*self.d_co_domain, 8*factor*self.d_co_domain, 16, 16, half_modes//4, half_modes//4, separable=separable)

        self.conv2_1 = SpectralConv2d(8*factor*self.d_co_domain, 16*factor*self.d_co_domain, 8, 8, half_modes//8, half_modes//8, separable=separable)

        self.conv2_9 = SpectralConv2d(16*factor*self.d_co_domain, 8*factor*self.d_co_domain, 16, 16, half_modes//8, half_modes//8, separable=separable)


        self.conv3 = SpectralConv2d(16*factor*self.d_co_domain, 4*factor*self.d_co_domain, 32, 32, half_modes//4, half_modes//4, separable=separable)

        self.conv4 = SpectralConv2d(8*factor*self.d_co_domain, 2*factor*self.d_co_domain, 48, 48, half_modes//2, half_modes//2, separable=separable)

        self.conv5 = SpectralConv2d(4*factor*self.d_co_domain, self.d_co_domain, 64, 64, half_modes, half_modes, separable=separable) # will be reshaped
        ...
        self.fc2 = nn.Linear(4*self.d_co_domain, out_ch)

    def forward(self, x):
        x = x.permute(0,2,3,1)
        grid = self.get_grid(x.shape, x.device)
        x = torch.cat((x, 0.*grid), dim=-1)
