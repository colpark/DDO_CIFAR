...
def save_model(args,
               ...
               gen_sde_optimizer,
               best_fid_score,
               checkpoint_name='checkpoint_fid.pt',
               ):
    ...
    if global_step > 0 and args.eval_use_ema and hasattr(gen_sde_optimizer, 'swap_parameters_with_ema'):
        checkpoint_name = checkpoint_name[:-3] + '_ema.pt'
...
def save_checkpoint(args, global_step, best_fid_score, gen_sde, gen_sde_optimizer, gen_sde_scheduler=None, checkpoint_name='checkpoint.pt'):
    if args.global_rank == 0:
        content = {'global_step': global_step,
                   'args': args,
                   'best_fid_score': best_fid_score,
...
def load_checkpoint(checkpoint_file, gen_sde, gen_sde_optimizer, gen_sde_scheduler=None, state_update_rule=None, strict=True):
...
class Logger(object):
    def __init__(self, rank, save, suffix=None):
        ...
        if self.rank == 0:
            if suffix is not None and isinstance(suffix, str):
                suffix_str = '_' + suffix
            elif suffix is not None and isinstance(suffix, list):
                suffix_str = '_' + '_'.join([str(i).lower() for i in suffix])
            else:
                suffix_str = ''
            log_format = '%(asctime)s %(message)s'
            logging.basicConfig(stream=sys.stdout, level=logging.INFO,
                                format=log_format, datefmt='%m/%d %I:%M:%S %p')
            fh = logging.FileHandler(os.path.join(save, 'log{}.txt'.format(suffix_str)))
    ...
    def info(self, string, *args):
        if self.rank == 0:
            cur_time = time.time()
            elapsed_time_from_start = cur_time - self.start_time
            elapsed_time_from_prev  = cur_time - self.prev_time
            self.prev_time = cur_time
            elapsed_time = '(Elapsed ' \
                    + time.strftime('from start: %H:%M:%S', time.gmtime(elapsed_time_from_start)) \
                    + ', ' + time.strftime('from prev: %H:%M:%S', time.gmtime(elapsed_time_from_prev)) \
                    + ') '
...
def common_init(rank, seed, save_dir, suffix=None):
    ...
    logging = Logger(rank, save_dir, suffix=suffix)
...
def gather_tensor(t, is_distributed):
    if is_distributed:
        t_all_gathered = [torch.zeros_like(t)] * dist.get_world_size()
        dist.all_gather(t_all_gathered, t)
        t_full = torch.cat(t_all_gathered, dim=0)
    else:
        t_full = t
    return t_full
